...
class NodeEncoder(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size=128, output_size=128):
        ...
        # TODO: if using historical information, change this to be an LSTM
...
class GNNModel(torch.nn.Module):
    ...
    # def forward(self, data):
    def forward(self, x, edge_attr, edge_index, u, batch, return_feature=False):
        '''
        edge_index,
        x,
        edge_attr,
        ...
        # TODO: if use temporal information, 
        # data.x and data.edge_attr needs to be sequences of batch data of shape (seq_length, batch_size, attr_dim)
        node_embedding, edge_embedding = self.dyn_models['encoder'](x, edge_attr)
        node_embedding, _, _ = self.dyn_models['processor'](node_embedding,
                                                               edge_index,
                                                               edge_embedding,
                                                               u=u,
                                                               batch=batch)
        out = self.dyn_models['decoder'](node_embedding)
