...
class SpongeFsm:
    ...
    def __init__(self, gym, sim, envs, env_id, cam_handles,
                 cam_props, sponge_actor, viewer, axes,state,
    ...
    def run_state_machine(self):
        ...
        particle_state_tensor = gymtorch.wrap_tensor(self.gym.acquire_particle_state_tensor(self.sim))
        ...
        if self.state == "capture_target_pc":
            if not os.path.exists(target_object_pc_file):
                ...
                pcd = open3d.geometry.PointCloud()
                pcd.points = open3d.utility.Vector3dVector(np.array(object_pc))
                open3d.io.write_point_cloud("/home/trannguyenle/RemoteWorkingStation/ros_workspaces/IsaacGym/isaacgym/python/robot_sponge/target_object_pc/"+str(self.env_id)+".pcd", pcd)
            else:
            ...
            self.F_curr = sim_utils.extract_net_forces(self.gym,self.sim)  
